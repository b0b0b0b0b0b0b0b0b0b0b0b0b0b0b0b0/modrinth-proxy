...
function translateDonationPlatform(platform, t) {
  const platforms = {
    Other: t('links.donate'),
    Patreon: 'Patreon',
    'Ko-fi': 'Ko-fi',
    PayPal: 'PayPal',
  ...
  return platforms[platform] || t('links.donate')
}
          ...
          <>
            ...
            {resource.donation_urls.map((donation, idx) => (
              <a key={idx} href={donation.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-300 hover:text-modrinth-green-light transition-colors group">
                ...
                <span className="group-hover:underline">{translateDonationPlatform(donation.platform, t)}</span>
                <ExternalIcon />
              </a>
            ))}
          </>
