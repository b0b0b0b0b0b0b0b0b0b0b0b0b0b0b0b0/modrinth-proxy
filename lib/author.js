import { normalizeProject, MODRINTH_SERVER_API } from './modrinth'

const MODRINTH_API = 'https://api.modrinth.com/v2';

async function fetchAuthorProjects(userId) {
...
async function fetchUserOrganizations(userId) {
  const url = `${MODRINTH_SERVER_API}/user/${userId}/organizations`;
...
async function collectAuthorProjects(userId) {
  const [directProjects, organizations] = await Promise.all([
    fetchAuthorProjects(userId),
    fetchUserOrganizations(userId),
  ]);

  const orgProjectLists = await Promise.all(
    organizations.map((org) => fetchOrganizationProjects(org.id)),
  );

  const seen = new Set();
  const merged = [];

  for (const list of [directProjects, ...orgProjectLists]) {
...
export async function getAuthorProjects(userId, { projectType = null, limit = 20, offset = 0 } = {}) {
  try {
    const projects = await collectAuthorProjects(userId);
...
export async function getUserOrganizations(userId) {
  try {
    ...
    return organizations.map((org) => org.id).filter(Boolean);
  } catch (error) {
