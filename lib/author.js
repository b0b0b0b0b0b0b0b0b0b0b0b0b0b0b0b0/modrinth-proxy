import { filterModsList } from './contentFilter'
...
import { ALL_LOADER_IDS } from './loaders'

const KNOWN_LOADERS = ALL_LOADER_IDS
...
export async function getAuthorProjects(userId, { projectType = null, limit = 20, offset = 0 } = {}) {
  try {
    ...
    const visibleProjects = (await filterModsList(rawProjects.map(normalizeAuthorProject))).hits

    let filteredProjects = visibleProjects
    if (projectType) {
      filteredProjects = visibleProjects.filter(
