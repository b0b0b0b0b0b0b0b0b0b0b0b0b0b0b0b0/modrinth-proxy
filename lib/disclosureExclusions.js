...
export function appendDisclosureExclusionFacets(facets, searchParams) {
  disclosureExclusionFacets(parseDisclosureExclusions(searchParams)).forEach((group) => {
    facets.push(group)
  })
}

export function appendDisclosureExclusionParams(params, source) {
  const ids = Array.isArray(source) ? source : parseDisclosureExclusions(source)
...
export function withDisclosureExclusionQuery(href, searchParams) {
  const ids = parseDisclosureExclusions(searchParams)
...
export function appendDisclosureExclusionNewFilters(parts, searchParams) {
