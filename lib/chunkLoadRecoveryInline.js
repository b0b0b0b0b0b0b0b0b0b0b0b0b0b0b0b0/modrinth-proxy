...
export const CHUNK_LOAD_RECOVERY_INLINE = `(function(){
  ...
  var RELOAD_COUNT_KEY = 'next-chunk-reload-count';
  var MIGRATE_KEY = 'pwa-no-chunk-precache-v5';
  var MAX_RELOADS = 3;
  ...
  function isChunkFailure(event) {
    ...
    return name === 'ChunkLoadError'
      ...
      || message.indexOf('Minified React error #423') !== -1;
  }

  function cleanRecoveryParams(url) {
  ...
  function hardReloadClean() {
    var url = cleanRecoveryParams(new URL(window.location.href));
  ...
  function recover() {
    ...
    var count = parseInt(sessionStorage.getItem(RELOAD_COUNT_KEY) || '0', 10);
    if (count >= MAX_RELOADS) {
      recovering = true;
      teardownSwCaches(hardReloadClean);
      return;
    }
    recovering = true;
