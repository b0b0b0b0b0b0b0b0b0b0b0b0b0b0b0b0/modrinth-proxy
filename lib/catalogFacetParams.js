import { canonicalizeLoaderId } from '@/lib/loaders'
...
function facetValueId(id) {
  return canonicalizeLoaderId(id)
}
...
export function parseFacetList(values) {
  ...
  for (const param of values || []) {
    ...
    if (decoded.startsWith(EXCLUDE)) {
      const id = facetValueId(decoded.slice(EXCLUDE.length))
      if (id && !excluded.includes(id)) excluded.push(id)
    } else if (decoded.startsWith(INCLUDE)) {
      const id = facetValueId(decoded.slice(INCLUDE.length))
